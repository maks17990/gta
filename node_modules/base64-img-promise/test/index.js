...
describe('Test', function () {
  ...
  it('All test (promise)', function (done) {
    var demo = readFileSync('img/demo.png');

    base64Img.base64(getpath('img/demo.png')).then(function (data) {
      base64Img.img(data, getpath('dest'), '1').then(function (filepath) {
        assert.equal(filepath, getpath('dest/1.png'));
        assert.equal(demo, readFileSync('dest/1.png'));
        done();
      });
  ...
  describe('img', function () {
    ...
    it('request (promise)', function (done) {
      var url = 'http://webresource.c-ctrip.com/ResCRMOnline/R5/html5/images/57.png';
      var demo = readFileSync('img/demo.png');
      var _data = base64Img.base64Sync(getpath('img/demo.png'));

      base64Img.requestBase64(url).then(function ({ res, data }) {
        assert.equal(data.substr(0, 10), _data.substr(0, 10));
        done();
      }).catch(function (err) {
        done();
        console.log(err);
